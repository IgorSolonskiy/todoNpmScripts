...
const renderElem = (listElem, elem) => {
  ...
  inputCheckboxElem.setAttribute("data-id", "".concat(elem.id));
  const btnDeleteElem = document.createElement("button");
  btnDeleteElem.classList.add("list-item__btnDelete");
  btnDeleteElem.setAttribute("data-id", "".concat(elem.id));
...
export const renderList = async () => {
  try {
    ...
    listElem.innerHTML = "";
    arrayTasks.sort((x, y) => new Date(x.createDate) - new Date(y.createDate)).sort((a, b) => a.done - b.done).forEach(elem => renderElem(listElem, elem));
    allInit();
  } catch (err) {
    alert(err.message);
  }
